...
ris_file_dat <- 
  purrr::map(embase_file_names, ~ {
    revtools::read_bibliography(filename = paste0("undermappe_til_arbejdssti_1/", .x)) |> 
...
ris_file_dat <- 
  purrr::map(embase_file_names, ~ {
    revtools::read_bibliography(filename = fs::path_package("extdata", .x, package = "VIVECampbell")) |> 
      base::suppressWarnings()
  }
  ) |> 
  purrr::list_rbind()
...
revtools::write_bibliography(ris_file_1_500, filename = "EMBASE_ris_file_1_500.ris", format = "ris")
revtools::write_bibliography(ris_file_501_1000, filename = "EMBASE_ris_file_501_1000.ris", format = "ris")
...
gem_ris <- function(seq, file_name, data){
  ...
  invisible(revtools::write_bibliography(ris_dat, filename = name_file, format = "ris"))
