...
Fisher1996 <- 
  ...
  mutate(
    n_dot = sum(N),
    sd_pool_grp = sqrt((1/n_dot)*(sum((N-1)*sd_post^2) + (prod(N)/n_dot)*(diff(m_post)^2)) ),
    .by = c(outcome, setting)
  ) |> 
  bind_rows(filter(Fisher1996, treatment == "TAU")) |> 
  mutate(
    n_dot = if_else(treatment == "TAU", N, n_dot),
    sd_pool_grp = if_else(treatment == "TAU", sd_post, sd_pool_grp)
  ) |> 
