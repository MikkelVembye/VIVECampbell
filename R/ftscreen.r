...
#' @param api_key Your OpenAI API key. Defaults to `get_api_key()`. Imported from AIscreenR package.
...
#' \item{price_dollar}{Total estimated cost of the screening (in dollars) based on actual token usage.}
#' \item{price_data}{A tibble with detailed price breakdown per prompt/model/iteration.}
#' 
...
price_data <- .price_gpt(combined_answer_data)
price_dollar <- sum(price_data$total_price_dollar, na.rm = TRUE)
